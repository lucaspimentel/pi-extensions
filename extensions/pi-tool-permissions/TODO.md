...
- [ ] Use `.pi/pi-tool-permissions.local.json` instead of `.pi/pi-tool-permissions.json` for project-scoped settings
  - Current project config constants in `index.ts` are `PROJECT_CONFIG_REL = join(".pi", "pi-tool-permissions.json")` and `LEGACY_PROJECT_CONFIG_REL = join(".pi", "tool-permissions.json")`; `loadProjectConfigRaw()` reads the new project file first and `saveProjectConfig()` writes it.
  - Update project-scoped reads/writes, docs, examples, and tests to prefer `.pi/pi-tool-permissions.local.json` for local machine/project settings while keeping a sensible fallback/migration path from `.pi/pi-tool-permissions.json` and legacy `.pi/tool-permissions.json`.
  - The current checkout already has an untracked `.pi/pi-tool-permissions.json` containing `allow: ["Bash(rg *)"]`; account for this when testing migration/rename behavior and consider documenting/ignoring the `.local.json` file if it should stay out of git.
